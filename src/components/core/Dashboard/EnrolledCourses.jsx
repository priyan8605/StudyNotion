...
export default function EnrolledCourses() {
  ...
  const getEnrolledCourses = async() => {
    try {
      const res = await getUserEnrolledCourses(token)  // Getting all the published and the drafted courses from the backend
      //getUserEnrolledCourses() is defined in "profileAPI.js" in "services/operations" which calls the backend API 
      //GET_USER_ENROLLED_COURSES_API: BASE_URL + "/profile/getEnrolledCourses", ko call karta hai jo ki present
      // hai in server=>controllers=>Profile.js me "getEnrolledCourses" function ke andr
...
//   }, [])
  ...
  return (
    <div>
      <div className="text-3xl text-white uppercase tracking-wider lg:text-left text-center">
      Enrolled Course
      </div>
      {!enrolledCourses ? (
